...
class ContinuationScriptContractTests(unittest.TestCase):
    ...
    def test_revision_dependencies_and_raw_contracts_are_explicit(self) -> None:
        ...
        self.assertIn("EUF_VIPER_BASE_DEPENDENCY_JOB_ID", submit)
        self.assertIn("--dependency=afterok:$BASE_DEPENDENCY_JOB_ID", submit)
        self.assertIn('"base_audit_job_id": int(base_audit)', submit)
        self.assertIn('"base_dependency_job_id": int(base_dependency)', submit)
