...
- [ ] Independently parse SMT-LIB and reconstruct the atom map and base Tseitin
  clauses before accepting EUF clauses or the SAT proof. This remains the main
  certificate gap.
...
- [ ] Ingest and hash the exact SMT-COMP 2025 QF_UF selection.
- [ ] Install, pin, and hash OpenSMT 2.9.2; repin Z3, cvc5, and Yices2 source and
  binaries.
- [ ] Add family/generator taxonomy, normalized-AST fingerprints, and grouped
  development/holdout splits. Random file-hash folds are forbidden.
- [ ] Add a declarative campaign lock that generates all child environments and
  rejects omitted causal variables.
- [ ] Add a cgroup/BenchExec-style runner with one-core binding, memory/RSS/CPU
  accounting, process-group termination, balanced solver order, and immutable
  resume.
- [ ] Add hierarchical family-cluster analysis: exact McNemar, PAR-2,
  family-macro score, 99% superiority intervals, and Holm correction.
...
1. Finish and validate P0 tooling.
2. Add OpenSMT, pin cvc5 exactly, and compare Z3 default with `sat.euf=true`.
3. Ablate Kissat 4.0.4 against the current SC2021 backend.
4. Produce current sound 2/60/1,200-second five-solver baselines.
