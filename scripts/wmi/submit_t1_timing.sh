...
CAMPAIGN_ROOT="$REMOTE_WORK/results/t1-typed-parser-timing-$CAMPAIGN_TAG"

ssh "$REMOTE_HOST" "set -euo pipefail; mkdir -p '$REMOTE_PARENT'; if [ ! -d '$REMOTE_WORK/.git' ]; then git clone --quiet https://github.com/nasqret/euf-viper.git '$REMOTE_WORK'; fi; git -C '$REMOTE_WORK' fetch --quiet origin '+refs/heads/$PUBLISHED_BRANCH:refs/remotes/origin/$PUBLISHED_BRANCH'; test \"\$(git -C '$REMOTE_WORK' rev-parse 'origin/$PUBLISHED_BRANCH^{commit}')\" = '$REVISION'; git -C '$REMOTE_WORK' checkout --quiet --detach '$REVISION'; test \"\$(git -C '$REMOTE_WORK' rev-parse HEAD)\" = '$REVISION'; test ! -e '$CAMPAIGN_ROOT'"

common_environment="EUF_VIPER_EXPECTED_REVISION='$REVISION' EUF_VIPER_T1_PUBLISHED_REF='$PUBLISHED_REF' EUF_VIPER_T1_TIMING_ROOT='$CAMPAIGN_ROOT' EUF_VIPER_PYTHON='$REMOTE_PYTHON' EUF_VIPER_PYTHON_SHA256='$REMOTE_PYTHON_SHA256' EUF_VIPER_PYTHON_VERSION='$REMOTE_PYTHON_VERSION' EUF_VIPER_CARGO='$REMOTE_CARGO' EUF_VIPER_CARGO_SHA256='$REMOTE_CARGO_SHA256' EUF_VIPER_CARGO_VERSION='$REMOTE_CARGO_VERSION' EUF_VIPER_RUSTC='$REMOTE_RUSTC' EUF_VIPER_RUSTC_SHA256='$REMOTE_RUSTC_SHA256' EUF_VIPER_RUSTC_VERSION='$REMOTE_RUSTC_VERSION'"

prepare_dependency=""
if [ -n "$DEPENDENCY" ]; then
  prepare_dependency="--dependency=afterok:$DEPENDENCY"
fi
PREPARE_SUBMISSION="$(ssh "$REMOTE_HOST" "cd '$REMOTE_WORK' && mkdir -p results && env $common_environment sbatch --parsable --partition='$PARTITION' $prepare_dependency --export=ALL scripts/wmi/euf_viper_t1_timing_prepare.sbatch")"
PREPARE_JOB="${PREPARE_SUBMISSION%%;*}"
case "$PREPARE_JOB" in ''|*[!0-9]*) echo "invalid prepare job id: $PREPARE_SUBMISSION" >&2; exit 2 ;; esac

LAST_SHARD="$((SHARDS - 1))"
ARRAY_SUBMISSION="$(ssh "$REMOTE_HOST" "cd '$REMOTE_WORK' && env $common_environment sbatch --parsable --partition='$PARTITION' --dependency=afterok:$PREPARE_JOB --array=0-$LAST_SHARD%$MAX_PARALLEL --export=ALL scripts/wmi/euf_viper_t1_timing_array.sbatch")"
ARRAY_JOB="${ARRAY_SUBMISSION%%;*}"
case "$ARRAY_JOB" in ''|*[!0-9]*) echo "invalid array job id: $ARRAY_SUBMISSION" >&2; exit 2 ;; esac

AUDIT_SUBMISSION="$(ssh "$REMOTE_HOST" "cd '$REMOTE_WORK' && env $common_environment sbatch --parsable --partition='$PARTITION' --dependency=afterok:$ARRAY_JOB --export=ALL scripts/wmi/euf_viper_t1_timing_audit.sbatch")"
