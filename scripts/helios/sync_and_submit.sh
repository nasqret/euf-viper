...
[ "$SHARDS" -le 1024 ] || die "shards cannot exceed 1024"
...
ssh -o BatchMode=yes -o ConnectTimeout=15 "$SSH_TARGET" \
  "RUN_ROOT='$RUN_ROOT' CAMPAIGN_ROOT='$CAMPAIGN_ROOT' REMOTE_ROOT='$REMOTE_ROOT' bash -l -s" <<'REMOTE'
set -euo pipefail
...
if [ "$MODE" = test-only ]; then
  printf 'mode=test-only execution_mode=%s shards=%s orchestration_revision=%s solver_revision=%s run_id=%s manifest_sha256=%s\n' \
    "$EXECUTION_MODE" "$SHARDS" "$ORCHESTRATION_REVISION" "$SOLVER_REVISION" "$RUN_ID" "$MANIFEST_SHA256"
  exit 0
fi
...
header=(
  schema_version status submitted_at job_id run_id execution_mode shard_count
  campaign_root orchestration_revision
...
values=(
  ...
  "$EXECUTION_MODE" "$SHARDS" "$CAMPAIGN_ROOT"
