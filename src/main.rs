...
use rustsat::solvers::{
    GetInternalStats, Learn, LimitConflicts, PhaseLit, Solve as RustSatSolve,
...
use rustsat_cadical::{
    CaDiCaL as CadicalSolver, Config as CadicalConfig,
    DecisionProbeSnapshot as CadicalDecisionProbeSnapshot,
    DecisionProbeState as CadicalDecisionProbeState,
};
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct CadicalBraidConfig {
    prefix_conflicts: u32,
    sprint_conflicts: u32,
    probe_min_decisions: u32,
    probe_max_decisions: u32,
    learn_max_len: u32,
    native_probe: bool,
}
...
struct CnfProblem {
    ...
    finite_cadical_braid_config: Option<CadicalBraidConfig>,
}
...
impl CnfProblem {
    fn new() -> Self {
        Self {
            ...
            finite_cadical_braid_config: None,
        }
    }
...
fn add_finite_domain_axioms_with_context(
    ...
    finite_dense7_braid_config: Option<CadicalBraidConfig>,
            ...
            || finite_symmetry_requested(arena.apps.len(), &symmetry_mode, symmetry_min_apps)
        {
            if force_finite_symmetry {
                ...
                if let Some(route) = finite_dense_cadical_candidate(
                    ...
                    finite_dense7_braid_config.is_some(),
                ) {
                    cnf.finite_cadical_search_hint = route.search_hint();
                    match route {
                        FiniteDenseCadicalRoute::Domain7Staged => {
                            cnf.finite_kissat_conflict_limit =
                                finite_dense7_staged_kissat_conflicts;
                        }
                        FiniteDenseCadicalRoute::Domain7Braided => {
                            cnf.finite_cadical_braid_config = Some(
                                finite_dense7_braid_config
                                    .expect("braided route requires configured limits"),
                            );
                        }
                        FiniteDenseCadicalRoute::Domain6
                        | FiniteDenseCadicalRoute::Domain7Direct => {}
                    }
...
impl FiniteDenseCadicalRoute {
    fn profile_label(self) -> &'static str {
        match self {
            ...
            Self::Domain7Braided => "finite_dense7_braided_route",
        }
    }
...
fn finite_dense_cadical_candidate(
    ...
    dense7_braided_enabled: bool,
) -> Option<FiniteDenseCadicalRoute> {
    if !dense6_enabled && !dense7_enabled && !dense7_staged_enabled && !dense7_braided_enabled {
        return None;
    }
...
fn finite_dense_cadical_route_for_signature(
    ...
    } else if dense7_braided_enabled
    ...
    {
        Some(FiniteDenseCadicalRoute::Domain7Braided)
...
#[derive(Debug, Clone, PartialEq, Eq)]
enum CadicalSolveStep {
    Solved(SolveResult),
    Interrupted,
    InvalidTheoryModel,
}

fn prepare_cadical_euf_solver(
    cnf: &CnfProblem,
    arena: &TermArena,
    eager_congruence: bool,
) -> Option<(CadicalSolver<'static, 'static>, Option<u32>)> {
    prepare_cadical_euf_solver_with_hint(
        cnf,
        arena,
        eager_congruence,
...
fn prepare_cadical_euf_solver_with_hint(
    cnf: &CnfProblem,
    arena: &TermArena,
    eager_congruence: bool,
    search_hint: CadicalSearchHint,
    ...
    configure_cadical_with_hint(&mut solver, prefer_unsat_search, search_hint)?;
    for clause in &cnf.clauses {
    ...
    Some((solver, conflict_limit))
}

fn solve_loaded_cadical_euf(
    solver: &mut CadicalSolver<'_, '_>,
    cnf: &CnfProblem,
    arena: &TermArena,
    true_term: TermId,
    false_term: TermId,
) -> Option<CadicalSolveStep> {
    let sat_start = Instant::now();
    ...
    match result {
        RustSatResult::Unsat => return Some(CadicalSolveStep::Solved(SolveResult::Unsat)),
        RustSatResult::Interrupted => return Some(CadicalSolveStep::Interrupted),
        RustSatResult::Sat => {}
    }
    ...
    if conflicts.is_empty() {
        Some(CadicalSolveStep::Solved(SolveResult::Sat))
    } else {
        Some(CadicalSolveStep::InvalidTheoryModel)
    }
}

fn profile_cadical_probe_stats(
    label: &str,
    solver: &CadicalSolver<'_, '_>,
    limit: u32,
    step: &CadicalSolveStep,
) {
    if env::var_os("EUF_VIPER_PROFILE").is_none() {
        return;
    }
    let outcome = match step {
        CadicalSolveStep::Solved(SolveResult::Sat) => "sat",
        CadicalSolveStep::Solved(SolveResult::Unsat) => "unsat",
        CadicalSolveStep::Solved(SolveResult::Unsupported(_)) => "unsupported",
        CadicalSolveStep::Interrupted => "interrupted",
        CadicalSolveStep::InvalidTheoryModel => "invalid-theory-model",
    };
    eprintln!(
        "profile_{label} outcome={outcome} limit={limit} conflicts={} decisions={} \
         propagations={} redundant={} irredundant={}",
        solver.conflicts(),
        solver.decisions(),
        solver.propagations(),
        solver.get_redundant(),
        solver.get_irredundant(),
    );
}

fn profile_cadical_native_probe(snapshot: CadicalDecisionProbeSnapshot) {
    if env::var_os("EUF_VIPER_PROFILE").is_none() {
        return;
    }
    let state = match snapshot.state {
        CadicalDecisionProbeState::Disabled => "disabled",
        CadicalDecisionProbeState::Armed => "armed",
        CadicalDecisionProbeState::Bypassed => "bypassed",
        CadicalDecisionProbeState::Selected => "selected",
    };
    eprintln!(
        "profile_finite_dense7_native_probe state={state} conflicts={} decisions={}",
        snapshot
            .conflicts
...
fn cadical_learned_clause_is_source_only(clause: &RustSatClause, var_count: usize) -> bool {
    clause.iter().all(|literal| literal.vidx() < var_count)
}

fn cadical_braid_probe_selected(decisions: usize, config: CadicalBraidConfig) -> bool {
    decisions >= config.probe_min_decisions as usize
        && decisions <= config.probe_max_decisions as usize
}

#[allow(clippy::too_many_arguments)]
fn profile_cadical_braid_gate(
    selected: bool,
    decisions: usize,
    config: CadicalBraidConfig,
    captured_clauses: usize,
    captured_literals: usize,
    rejected_clauses: usize,
) {
    if env::var_os("EUF_VIPER_PROFILE").is_some() {
        eprintln!(
            "profile_finite_dense7_braided_gate selected={} decisions={decisions} min={} max={} \
             learn_max_len={} captured_clauses={captured_clauses} \
             captured_literals={captured_literals} rejected_clauses={rejected_clauses}",
            usize::from(selected),
            config.probe_min_decisions,
            config.probe_max_decisions,
            config.learn_max_len,
        );
    }
}
...
fn solve_cadical_euf_once(
    ...
    let (mut solver, conflict_limit) = prepare_cadical_euf_solver(cnf, arena, eager_congruence)?;

    if let Some(limit) = conflict_limit {
        solver.limit_conflicts(Some(limit)).ok()?;
    }
    match solve_loaded_cadical_euf(&mut solver, cnf, arena, true_term, false_term)? {
        CadicalSolveStep::Solved(result) => Some(result),
        CadicalSolveStep::Interrupted | CadicalSolveStep::InvalidTheoryModel => None,
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct BraidedSolveOutcome {
    result: Option<SolveResult>,
    sat_calls: usize,
}

fn solve_cadical_native_phase_braid(
    cnf: &CnfProblem,
    arena: &TermArena,
    true_term: TermId,
    false_term: TermId,
    ...
    let Some((mut solver, resume_limit)) = prepare_cadical_euf_solver(cnf, arena, eager_congruence)
    else {
        return BraidedSolveOutcome {
            result: None,
            sat_calls: 0,
        };
    };
    if let Some(limit) = resume_limit {
        if solver.limit_conflicts(Some(limit)).is_err() {
            return BraidedSolveOutcome {
                result: None,
                sat_calls: 0,
            };
    ...
    if solver
        .configure_decision_probe(
            u64::from(config.prefix_conflicts),
            u64::from(config.probe_min_decisions),
            u64::from(config.probe_max_decisions),
        )
        .is_err()
    {
        return BraidedSolveOutcome {
    ...
    let Some(probe_step) = solve_loaded_cadical_euf(&mut solver, cnf, arena, true_term, false_term)
    else {
        return BraidedSolveOutcome {
            result: None,
            sat_calls: 1,
    ...
    let Ok(snapshot) = solver.decision_probe_snapshot() else {
        return BraidedSolveOutcome {
            result: None,
            sat_calls: 1,
        };
    };
    profile_cadical_native_probe(snapshot);
    profile_measurement(
        "finite_dense7_native_probe_conflicts",
        u128::from(snapshot.conflicts.unwrap_or(0)),
        config.prefix_conflicts as usize,
    );
    profile_measurement(
        "finite_dense7_native_probe_decisions",
        u128::from(snapshot.decisions.unwrap_or(0)),
        config.probe_max_decisions as usize,
    );
    match probe_step {
        CadicalSolveStep::Solved(result) => {
            profile_measurement("finite_dense7_braided_handoff", 0, 0);
            return BraidedSolveOutcome {
                result: Some(result),
                sat_calls: 1,
            };
        }
        CadicalSolveStep::InvalidTheoryModel => {
            return BraidedSolveOutcome {
                result: None,
                sat_calls: 1,
            };
        }
        CadicalSolveStep::Interrupted if snapshot.state != CadicalDecisionProbeState::Selected => {
            return resume_cadical_phase_braid(
                solver,
                resume_limit,
                cnf,
                arena,
        ...
        CadicalSolveStep::Interrupted => {}
    }

    let sprint_limit = config.sprint_conflicts;
    profile_measurement(
        "finite_dense7_braided_cadical_sprint_budget",
        u128::from(sprint_limit),
        cnf.clauses.len(),
    );
    let mut sat_calls = 1usize;
    if let Some((mut sprint_solver, _)) = prepare_cadical_euf_solver_with_hint(
        cnf,
        arena,
        eager_congruence,
        CadicalSearchHint::UnsatSafe,
    ) {
        if sprint_solver.limit_conflicts(Some(sprint_limit)).is_err() {
            return resume_cadical_phase_braid(
                solver,
                resume_limit,
                cnf,
        ...
        let sprint_start_conflicts = sprint_solver.conflicts();
        if let Some(step) =
            solve_loaded_cadical_euf(&mut sprint_solver, cnf, arena, true_term, false_term)
        {
            sat_calls += 1;
            let sprint_conflicts = sprint_solver
                .conflicts()
                .saturating_sub(sprint_start_conflicts);
            profile_measurement(
                "finite_dense7_braided_cadical_sprint",
                sprint_conflicts as u128,
                sprint_limit as usize,
            );
            if let CadicalSolveStep::Solved(result) = step {
                profile_measurement("finite_dense7_braided_handoff", 1, 0);
                return BraidedSolveOutcome {
                    result: Some(result),
                    sat_calls,
                };
            }
            profile_measurement("finite_dense7_braided_handoff", 2, sprint_conflicts);
        }
    }

    resume_cadical_phase_braid(
        solver,
        resume_limit,
        cnf,
        arena,
...
fn solve_cadical_phase_braid(
    ...
    const MAX_CAPTURED_LEARNED_CLAUSES: usize = 4_096;
    let Some(config) = cnf.finite_cadical_braid_config else {
        return BraidedSolveOutcome {
            result: None,
            sat_calls: 0,
        };
    ...
    if config.native_probe {
        return solve_cadical_native_phase_braid(
            cnf,
            arena,
            true_term,
            false_term,
    ...
    let learned_clauses = Rc::new(RefCell::new(Vec::<RustSatClause>::new()));
    if config.learn_max_len > 0 {
        let learned_clauses_sink = Rc::clone(&learned_clauses);
        solver.attach_learner(
            move |clause| {
                if let Ok(mut clauses) = learned_clauses_sink.try_borrow_mut() {
                    if clauses.len() < MAX_CAPTURED_LEARNED_CLAUSES {
                        clauses.push(clause);
                    }
                }
            },
            config.learn_max_len as usize,
        );
    }
    let prefix_limit = config.prefix_conflicts;
    if solver.limit_conflicts(Some(prefix_limit)).is_err() {
        return BraidedSolveOutcome {
            result: None,
            sat_calls: 0,
        };
    }
    let prefix_start_conflicts = solver.conflicts();
    let Some(prefix) = solve_loaded_cadical_euf(&mut solver, cnf, arena, true_term, false_term)
    else {
        return BraidedSolveOutcome {
            result: None,
            sat_calls: 1,
    ...
    profile_measurement(
        "finite_dense7_braided_cadical_prefix",
        solver.conflicts().saturating_sub(prefix_start_conflicts) as u128,
        prefix_limit as usize,
    );
    profile_cadical_probe_stats(
        "finite_dense7_braided_probe",
        &solver,
        prefix_limit,
        &prefix,
    );
    if config.learn_max_len > 0 {
        solver.detach_learner();
    }
    match prefix {
        CadicalSolveStep::Solved(result) => {
            profile_measurement("finite_dense7_braided_handoff", 0, 0);
    ...
    let probe_selected = cadical_braid_probe_selected(decisions, config);
    let (seed_clauses, captured_clauses, captured_literals, rejected_clauses) = {
        let captured = learned_clauses.borrow();
        let captured_clauses = captured.len();
        let captured_literals = captured.iter().map(|clause| clause.len()).sum();
        let rejected_clauses = captured
            .iter()
            .filter(|clause| !cadical_learned_clause_is_source_only(clause, cnf.var_count()))
            .count();
        let seed_clauses: Vec<RustSatClause> = probe_selected
            .then(|| {
                captured
                    .iter()
                    .filter(|clause| cadical_learned_clause_is_source_only(clause, cnf.var_count()))
    ...
    if probe_selected {
        if let Some((mut sprint_solver, _)) = prepare_cadical_euf_solver_with_hint(
            ...
            let seed_clause_count = seed_clauses.len();
            let seed_literal_count = seed_clauses
                .iter()
                .map(|clause| clause.len())
                .sum::<usize>();
            let seed_load_ok = seed_clauses
                .into_iter()
                .all(|clause| sprint_solver.add_clause(clause).is_ok());
            profile_measurement(
                "finite_dense7_braided_seed_load",
                seed_clause_count as u128,
                seed_literal_count,
            );
            if !seed_load_ok || sprint_solver.limit_conflicts(Some(sprint_limit)).is_err() {
                return resume_cadical_phase_braid(
                    solver,
                    resume_limit,
                    cnf,
                ...
                solve_loaded_cadical_euf(&mut sprint_solver, cnf, arena, true_term, false_term)
            {
                ...
                profile_measurement(
                    "finite_dense7_braided_handoff",
                    2,
                    sprint_conflicts as usize,
                );
            }
        }
    }
...
#[allow(clippy::too_many_arguments)]
fn resume_cadical_phase_braid(
    mut solver: CadicalSolver<'static, 'static>,
    ...
    let limit_result = match resume_limit {
        Some(limit) => solver.limit_conflicts(Some(limit)),
        None => solver.limit_conflicts(None),
    };
    ...
    let result =
        solve_loaded_cadical_euf(&mut solver, cnf, arena, true_term, false_term).and_then(|step| {
            match step {
                CadicalSolveStep::Solved(result) => Some(result),
                CadicalSolveStep::Interrupted | CadicalSolveStep::InvalidTheoryModel => None,
            }
...
const FINITE_DENSE7_BRAIDED_ENV: &str = "EUF_VIPER_FINITE_DENSE7_BRAIDED";
const FINITE_DENSE7_NATIVE_PROBE_ENV: &str = "EUF_VIPER_FINITE_DENSE7_NATIVE_PROBE";
const FINITE_DENSE7_KISSAT_CONFLICTS_ENV: &str = "EUF_VIPER_FINITE_DENSE7_KISSAT_CONFLICTS";
const FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS_ENV: &str =
    "EUF_VIPER_FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS";
const FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS_ENV: &str =
    "EUF_VIPER_FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS";
const FINITE_DENSE7_PROBE_MIN_DECISIONS_ENV: &str = "EUF_VIPER_FINITE_DENSE7_PROBE_MIN_DECISIONS";
const FINITE_DENSE7_PROBE_MAX_DECISIONS_ENV: &str = "EUF_VIPER_FINITE_DENSE7_PROBE_MAX_DECISIONS";
const FINITE_DENSE7_LEARN_MAX_LEN_ENV: &str = "EUF_VIPER_FINITE_DENSE7_LEARN_MAX_LEN";
const DEFAULT_FINITE_DENSE7_KISSAT_CONFLICTS: u32 = 1_000;
const DEFAULT_FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS: u32 = 1_000;
const DEFAULT_FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS: u32 = 70_000;
const DEFAULT_FINITE_DENSE7_PROBE_MIN_DECISIONS: u32 = 0;
const DEFAULT_FINITE_DENSE7_PROBE_MAX_DECISIONS: u32 = u32::MAX;
const DEFAULT_FINITE_DENSE7_LEARN_MAX_LEN: u32 = 0;
...
impl RootCnfOptions {
    ...
    fn existing_behavior(direct_root_cnf: bool) -> Self {
        Self {
            ...
            finite_dense7_braid_config: None,
...
fn finite_dense7_braided_enabled() -> Result<bool, String> {
    zero_one_env_setting(FINITE_DENSE7_BRAIDED_ENV, false)
}

fn finite_dense7_native_probe_enabled() -> Result<bool, String> {
    zero_one_env_setting(FINITE_DENSE7_NATIVE_PROBE_ENV, false)
}
...
fn finite_dense7_cadical_prefix_conflicts() -> Result<u32, String> {
    match env::var(FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS_ENV) {
        Ok(value) => parse_nonnegative_u32_setting(
            FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS_ENV,
            Some(&value),
            DEFAULT_FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS,
        ),
        Err(env::VarError::NotPresent) => parse_nonnegative_u32_setting(
            FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS_ENV,
            None,
        ...
        Err(env::VarError::NotUnicode(_)) => Err(format!(
            "{FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS_ENV} must be a nonnegative 32-bit integer"
        )),
    }
}

fn finite_dense7_cadical_sprint_conflicts() -> Result<u32, String> {
    match env::var(FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS_ENV) {
        Ok(value) => parse_nonnegative_u32_setting(
            FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS_ENV,
            Some(&value),
            DEFAULT_FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS,
        ),
        Err(env::VarError::NotPresent) => parse_nonnegative_u32_setting(
            FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS_ENV,
            None,
        ...
        Err(env::VarError::NotUnicode(_)) => Err(format!(
            "{FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS_ENV} must be a nonnegative 32-bit integer"
        )),
    }
}

fn nonnegative_u32_env_setting(name: &str, default: u32) -> Result<u32, String> {
    match env::var(name) {
        Ok(value) => parse_nonnegative_u32_setting(name, Some(&value), default),
        Err(env::VarError::NotPresent) => parse_nonnegative_u32_setting(name, None, default),
        Err(env::VarError::NotUnicode(_)) => {
            Err(format!("{name} must be a nonnegative 32-bit integer"))
        }
    }
}

fn finite_dense7_probe_min_decisions() -> Result<u32, String> {
    nonnegative_u32_env_setting(
        FINITE_DENSE7_PROBE_MIN_DECISIONS_ENV,
        DEFAULT_FINITE_DENSE7_PROBE_MIN_DECISIONS,
    )
}

fn finite_dense7_probe_max_decisions() -> Result<u32, String> {
    nonnegative_u32_env_setting(
        FINITE_DENSE7_PROBE_MAX_DECISIONS_ENV,
        DEFAULT_FINITE_DENSE7_PROBE_MAX_DECISIONS,
    )
}

fn finite_dense7_learn_max_len() -> Result<u32, String> {
    let max_len = nonnegative_u32_env_setting(
        FINITE_DENSE7_LEARN_MAX_LEN_ENV,
        DEFAULT_FINITE_DENSE7_LEARN_MAX_LEN,
    )?;
    if max_len > i32::MAX as u32 {
        return Err(format!(
            "{FINITE_DENSE7_LEARN_MAX_LEN_ENV} must not exceed {}",
            i32::MAX
        ));
    }
    Ok(max_len)
...
fn selected_root_cnf_options() -> Result<RootCnfOptions, String> {
    ...
    let finite_dense7_braided = finite_dense7_braided_enabled()?;
    let finite_dense7_native_probe = finite_dense7_native_probe_enabled()?;
    if usize::from(finite_dense7_cadical)
        + usize::from(finite_dense7_staged)
        + usize::from(finite_dense7_braided)
        > 1
    {
        return Err(format!(
            "{FINITE_DENSE7_CADICAL_ENV}, {FINITE_DENSE7_STAGED_ENV}, and \
             {FINITE_DENSE7_BRAIDED_ENV} are mutually exclusive"
        ));
    }
    if finite_dense7_native_probe && !finite_dense7_braided {
        return Err(format!(
            "{FINITE_DENSE7_NATIVE_PROBE_ENV}=1 requires {FINITE_DENSE7_BRAIDED_ENV}=1"
        ));
    }
    let finite_dense7_kissat_conflicts = finite_dense7_kissat_conflicts()?;
    let finite_dense7_cadical_prefix_conflicts = finite_dense7_cadical_prefix_conflicts()?;
    let finite_dense7_cadical_sprint_conflicts = finite_dense7_cadical_sprint_conflicts()?;
    let finite_dense7_probe_min_decisions = finite_dense7_probe_min_decisions()?;
    let finite_dense7_probe_max_decisions = finite_dense7_probe_max_decisions()?;
    let finite_dense7_learn_max_len = finite_dense7_learn_max_len()?;
    if finite_dense7_probe_min_decisions > finite_dense7_probe_max_decisions {
        return Err(format!(
            "{FINITE_DENSE7_PROBE_MIN_DECISIONS_ENV} must not exceed \
             {FINITE_DENSE7_PROBE_MAX_DECISIONS_ENV}"
        ));
    }
    if finite_dense7_native_probe && finite_dense7_learn_max_len != 0 {
        return Err(format!(
            "{FINITE_DENSE7_LEARN_MAX_LEN_ENV} must be 0 when \
             {FINITE_DENSE7_NATIVE_PROBE_ENV}=1"
        ));
    }
    Ok(RootCnfOptions {
        ...
        finite_dense7_braid_config: finite_dense7_braided.then_some(CadicalBraidConfig {
            prefix_conflicts: finite_dense7_cadical_prefix_conflicts,
            sprint_conflicts: finite_dense7_cadical_sprint_conflicts,
            probe_min_decisions: finite_dense7_probe_min_decisions,
            probe_max_decisions: finite_dense7_probe_max_decisions,
            learn_max_len: finite_dense7_learn_max_len,
            native_probe: finite_dense7_native_probe,
        }),
        force_finite_predicate_channeling: false,
        force_finite_permutation_support: false,
    })
    ...
    if matches!(
            ...
            || env::var("EUF_VIPER_FINITE_DOMAIN").as_deref() == Ok("1")
        {
            ...
            let added = add_finite_domain_axioms_with_context(
                ...
                root_cnf_options.finite_dense7_braid_config,
        ...
        {
            let outcome = solve_cadical_phase_braid(
                &cnf,
                arena,
                bool_problem.true_term,
                bool_problem.false_term,
            ...
            auto_prior_sat_calls = outcome.sat_calls;
            ...
            && cnf.finite_cadical_search_hint == CadicalSearchHint::StagedUnsatSafe
        {
...
mod tests {
    ...
    fn pigeonhole_cnf(pigeons: usize, holes: usize) -> CnfProblem {
        let mut cnf = CnfProblem::new();
        let variables = (0..pigeons)
            .map(|_| (0..holes).map(|_| cnf.new_var(None)).collect::<Vec<_>>())
            .collect::<Vec<_>>();
        for pigeon in &variables {
            cnf.add_clause(pigeon.clone());
        }
        for hole in 0..holes {
            for left in 0..pigeons {
                for right in left + 1..pigeons {
                    cnf.add_clause(vec![-variables[left][hole], -variables[right][hole]]);
                }
            }
        }
        cnf.finite_cadical_search_hint = CadicalSearchHint::BraidedPlainSafe;
    ...
    #[test]
    fn cadical_conflict_prefix_can_resume_the_same_solver_state() {
        let cnf = pigeonhole_cnf(8, 7);
        ...
        assert_eq!(
            solve_loaded_cadical_euf(&mut solver, &cnf, &arena, 0, 0),
            Some(CadicalSolveStep::Interrupted),
        );
        let prefix_conflicts = solver.conflicts();
        solver.limit_conflicts(None).unwrap();
        assert_eq!(
            solve_loaded_cadical_euf(&mut solver, &cnf, &arena, 0, 0),
            Some(CadicalSolveStep::Solved(SolveResult::Unsat)),
        );
        assert!(solver.conflicts() >= prefix_conflicts);
    }

    #[test]
    fn cadical_phase_braid_can_solve_in_lazy_unsat_sprint() {
        let mut cnf = pigeonhole_cnf(8, 7);
        cnf.finite_cadical_braid_config = Some(CadicalBraidConfig {
            prefix_conflicts: 0,
            sprint_conflicts: 10_000,
            probe_min_decisions: 0,
            probe_max_decisions: u32::MAX,
            learn_max_len: 8,
            native_probe: false,
        });
        let outcome = solve_cadical_phase_braid(&cnf, &TermArena::default(), 0, 0, false);
        assert_eq!(outcome.result, Some(SolveResult::Unsat));
        assert_eq!(outcome.sat_calls, 2);
    }

    #[test]
    fn cadical_native_probe_bypasses_inside_one_solve_call() {
        let mut cnf = pigeonhole_cnf(8, 7);
        cnf.finite_cadical_braid_config = Some(CadicalBraidConfig {
            prefix_conflicts: 0,
            sprint_conflicts: 10_000,
            probe_min_decisions: 1,
            probe_max_decisions: 1,
            learn_max_len: 0,
            native_probe: true,
        });
        let outcome = solve_cadical_phase_braid(&cnf, &TermArena::default(), 0, 0, false);
    ...
    #[test]
    fn cadical_native_probe_can_select_the_unsat_sprint() {
        let mut cnf = pigeonhole_cnf(8, 7);
        cnf.finite_cadical_braid_config = Some(CadicalBraidConfig {
            prefix_conflicts: 0,
            sprint_conflicts: 10_000,
            probe_min_decisions: 0,
            probe_max_decisions: 0,
            learn_max_len: 0,
            native_probe: true,
        });
        let outcome = solve_cadical_phase_braid(&cnf, &TermArena::default(), 0, 0, false);
    ...
    #[test]
    fn cadical_phase_braid_probe_gate_has_inclusive_bounds() {
        let config = CadicalBraidConfig {
            prefix_conflicts: 100,
            sprint_conflicts: 70_000,
            probe_min_decisions: 131,
            probe_max_decisions: 150,
            learn_max_len: 8,
            native_probe: false,
        };
        assert!(!cadical_braid_probe_selected(130, config));
        assert!(cadical_braid_probe_selected(131, config));
        assert!(cadical_braid_probe_selected(150, config));
        assert!(!cadical_braid_probe_selected(151, config));
    }

    #[test]
    fn cadical_seed_filter_rejects_non_source_variables() {
        let clause = rustsat_clause(&[1, -3]);
        assert!(cadical_learned_clause_is_source_only(&clause, 3));
        assert!(!cadical_learned_clause_is_source_only(&clause, 2));
    }

    #[test]
    fn cadical_detach_learner_releases_the_callback() {
        let marker = Rc::new(());
        let mut solver = CadicalSolver::default();
        let callback_marker = Rc::clone(&marker);
        solver.attach_learner(
            move |_| {
    ...
    fn finite_rook_symmetry_requires_a_complete_regular_guard_graph() {
        ...
        assert_eq!(
            finite_dense_cadical_route_for_signature(7, 0, 147, 315, 0, false, false, true, false,),
            Some(FiniteDenseCadicalRoute::Domain7Staged),
        );
        assert_eq!(
            finite_dense_cadical_route_for_signature(7, 0, 148, 315, 0, false, false, true, false,),
            None,
        );
        assert_eq!(
            finite_dense_cadical_route_for_signature(7, 0, 147, 314, 0, false, false, true, false,),
            None,
        );
        assert_eq!(
            finite_dense_cadical_route_for_signature(7, 0, 147, 315, 0, false, false, false, true,),
            Some(FiniteDenseCadicalRoute::Domain7Braided),
        );
        assert_eq!(
            finite_dense_cadical_route_for_signature(7, 0, 148, 315, 0, false, false, false, true,),
            None,
        );
        assert_eq!(
        ...
        assert_eq!(
            parse_zero_one_setting_with_default(FINITE_DENSE7_BRAIDED_ENV, None, false),
            Ok(false),
        );
        assert_eq!(
            parse_zero_one_setting_with_default(FINITE_DENSE7_NATIVE_PROBE_ENV, None, false),
            Ok(false),
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Ok(DEFAULT_FINITE_DENSE7_CADICAL_PREFIX_CONFLICTS),
        );
        assert_eq!(
            parse_nonnegative_u32_setting(
                FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS_ENV,
            ...
            Ok(DEFAULT_FINITE_DENSE7_CADICAL_SPRINT_CONFLICTS),
        );
