...
| [Automated Reencoding Meets Graph Theory](https://doi.org/10.4230/LIPIcs.SAT.2026.29) | BVA has a graph-theoretic characterization and hard limits: even idealized BVA cannot recover the asymptotically smaller product encoding for at-most-one | Measure Viper's one-hot binary-clause graphs, generic BVA, and direct product/commander encodings on the same finite populations | Select a source-level encoding from proved finite-domain structure before CNF erases the discrete semantics | Direct encoding must beat generic BVA end to end and preserve checked model reconstruction |
...
| [Yices 2.2](https://yices.csl.sri.com/papers/cav2014.pdf) and [current context options](https://yices.csl.sri.com/doc/context-operations.html#preprocessing-options) | Yices combines CDCL with explanation-producing congruence closure, dynamic Ackermannization, learned-equality preprocessing, and QF_UF symmetry breaking | Keep Yices as the primary architecture and speed control; ablate its public `learn-eq` and `break-symmetries` options where the CLI/API permits | Viper must win through semantic finite routing, quotient-state reuse, or proof reconfiguration rather than relabeling known Yices ingredients | Every claimed mechanism must beat default Yices and the closest option ablation on an inclusive target panel |
